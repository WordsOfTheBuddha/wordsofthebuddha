...
export async function persistToFirestore(rangyHash: string, highlighter: Highlighter | null): Promise<void> {
    const validContainers = ["p", "div", "h1", "h2", "h3", "h4", "h5", "h6", "li", "table"];
    ...
    const highlight = {
        ...
        description: document.getElementById("highlight-root")?.dataset.description || "",
        rangyHash,
        highlightSegments,
        updatedAt: new Date(),
    };
...
export async function fetchHighlights(highlighter: Highlighter | null): Promise<void> {
    try {
        const slug = getSlug();
