...
export const POST: APIRoute = async ({ request, cookies }) => {
	...
	try {
		...
		(
			Object.keys(preferenceValidators) as Array<
				keyof typeof preferenceValidators
			>
		).forEach((key) => {
			...
			if (value !== undefined && value !== null) {
				try {
					updates[`preferences.${key}`] =
						preferenceValidators[key](value);
					hasValidUpdate = true;
				} catch (error: any) {
		...
		if (!hasValidUpdate) {
			return new Response("No valid preference updates provided", {
				status: 400,
			});
		}
