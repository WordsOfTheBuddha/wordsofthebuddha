...
function cleanupDpdHtml(html: string): string {
    ...
    const selectorsToRemove = [
        '.box-footer', '.comments', '.button-box', 'script', 'style',
        '.metadata', '.box-title', '.deconstructor_footer',
        '[id^="grammar_dhamma_"]', '[id^="examples_dhamma_"]',
        '[id^="declension_dhamma_"]', '[id^="family_root_dhamma_"]',
        '[id^="family_compound_dhamma_"]', '[id^="family_idiom_dhamma_"]',
        '[id^="frequency_dhamma_"]', '[id^="feedback_dhamma_"]',
        '.button_box',
    ];
...
async function lookupWord(word: string, summaryOnly = false): Promise<DpdResponse | null> {
    try {
        // Try cache first
...
export const GET: APIRoute = async ({ url }) => {
    ...
    if (query) {
        ...
        try {
            ...
            return new Response(JSON.stringify({ error: 'Batch lookup failed' }), { status: 500 });
        }
    }
