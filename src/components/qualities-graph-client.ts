...
// Layout tuning knobs
const BASE_LAYOUT = { idealEdgeLength: 220, nodeRepulsion: 80000, gravity: 1.0 } as const;
const FOCUS_LAYOUT = { minNodeSpacing: 18, spacingFactor: 1.15, animMs: 380, padding: 44 } as const;
...
const cy = cytoscape({
    ...
    style: [
        ...
        { selector: "edge", style: { "z-index-compare": "manual", "z-index": 2 } },
        {
            selector: "node", style: {
                "background-color": (el) =>
                    el.data("polarity") === "positive" ? "#10b981" :
                        el.data("polarity") === "negative" ? "#ef4444" : "#6366f1",
                label: "data(label)", "font-size": 15, "text-wrap": "wrap",
                "text-max-width": 260, "text-valign": "center", color: "#111827",
                width: 36, height: 36,
        ...
        {
            selector: "edge[type = 'supportedBy'][?dir], edge[type = 'leadsTo'][?dir], edge[type = 'guardedBy'][?dir]",
            style: {
                "curve-style": "bezier", width: 1.4,
                "line-color": (e) => e.data("type") === "supportedBy" ? "#059669" : e.data("type") === "leadsTo" ? "#2563eb" : "#7c3aed",
                "target-arrow-shape": "triangle",
                "target-arrow-color": (e) => e.data("type") === "supportedBy" ? "#059669" : e.data("type") === "leadsTo" ? "#2563eb" : "#7c3aed",
                "arrow-scale": 0.8, opacity: 0.9
            }
        },
        // Perspective overrides for edges adjacent to the focused node
        { selector: "edge.from-focus[type = 'supportedBy'], edge.from-focus[type = 'leadsTo'], edge.from-focus[type = 'guardedBy']", style: { "line-color": "#2563eb", "target-arrow-color": "#2563eb", "z-index": 1000 } },
        { selector: "edge.to-focus[type = 'supportedBy'], edge.to-focus[type = 'leadsTo'], edge.to-focus[type = 'guardedBy']", style: { "line-color": "#059669", "target-arrow-color": "#059669", "z-index": 1000 } },
        {
            selector: "edge[type = 'related'], edge[type = 'opposite']",
            style: {
                "curve-style": "bezier", width: 1.0,
                "line-style": (e) => e.data("type") === "related" ? "solid" :
                    e.data("type") === "opposite" ? "dashed" : "solid",
                "line-color": (e) => e.data("type") === "related" ? "#eab308" : "#ef4444",
                opacity: .6, "target-arrow-shape": "none"
            }
        },
        { selector: ".hidden", style: { display: "none" } },
        { selector: ".dimmed", style: { opacity: .15 } },
        { selector: ".highlight", style: { "border-width": 3, "border-color": "#111827" } },
    ],
    // Increase spacing to reduce label collisions in the base layout
    layout: { name: "cose", animate: false, idealEdgeLength: BASE_LAYOUT.idealEdgeLength, nodeRepulsion: BASE_LAYOUT.nodeRepulsion, gravity: BASE_LAYOUT.gravity, nodeDimensionsIncludeLabels: true },
    minZoom: 0.05, maxZoom: 4.0, wheelSensitivity: 0.22
});
...
cy.ready(() => {
    ...
    if (container) {
        cy.on('mouseover', 'node', () => { (container as HTMLElement).style.cursor = 'pointer'; });
        cy.on('mouseout', 'node', () => { (container as HTMLElement).style.cursor = ''; });
    }
});
...
(function loadOpts(): void {
    try {
        const raw = document.getElementById("qualities-map-options")?.textContent || "{}";
...
type SearchEntry = { id: string; slug: string; label: string; ascii: string; tokens: string[] };
...
const SEARCH_INDEX: SearchEntry[] = (() => {
    ...
    built.nodes.forEach((n: any) => {
        ...
        for (const raw of lines) {
            ...
            if (/^\s*(Supported by|Leads to|Related|Related to|Opposite|Guarded by|Context)\s*:/i.test(s)) continue;
            const pm = s.match(/^\s*\[([^\]]+)\]\s*$/);
            if (pm) { pm[1].split(',').forEach(t => tokens.push(t.trim())); continue; }
            // split comma-separated synonyms into individual tokens as well as full phrase
            tokens.push(s);
            if (s.includes(',')) s.split(',').forEach(p => tokens.push(p.trim()));
        }
...
function searchEntries(query: string): SearchEntry[] {
    ...
    if (exact.length) {
        console.debug('[QM][search] exact token hits=', exact.map(e => e.id));
        return exact;
    }
    // starts-with token priority
    const starts = SEARCH_INDEX.filter(e => e.tokens.some(t => t.startsWith(q)));
    if (starts.length) {
        console.debug('[QM][search] starts-with hits=', starts.map(e => e.id));
        return starts;
    }
    // contains in ascii blob
    const contains = SEARCH_INDEX.filter(e => e.ascii.includes(q));
    if (contains.length) {
        console.debug('[QM][search] contains hits=', contains.map(e => e.id));
        return contains;
    }
    // fuzzy (edit distance <= 1) on tokens
    const d1: SearchEntry[] = [];
    for (const e of SEARCH_INDEX) {
        if (e.tokens.some(t => t === q || Math.abs(t.length - q.length) <= 1 && editDistanceLe1(t, q))) {
            d1.push(e);
        }
    }
    if (d1.length) console.debug('[QM][search] fuzzy<=1 hits=', d1.map(e => e.id));
    return d1;
}

function pickPreferredIfAmbiguous(query: string, results: SearchEntry[]): SearchEntry | null {
    const q = toAscii(query);
    const preferred = PREFERRED_SYNONYM_TARGET[q];
    if (!preferred) return null;
    // Try to find a result whose label or slug matches the preferred target
    const hit = results.find(e => toAscii(e.label) === preferred || toAscii(e.slug) === preferred || toAscii(e.id) === preferred);
    return hit || null;
}
...
function setUrlFocus(slug: string | null, push = false) {
    try {
        ...
        if (slug) u.searchParams.set("focus", slug); else u.searchParams.delete("focus");
        if (push) history.pushState({ focus: slug }, "", u.toString()); else history.replaceState({ focus: slug }, "", u.toString());
    } catch { /* noop */ }
}
function getUrlExpanded(): boolean {
    try {
        const u = new URL(location.href);
        const v = (u.searchParams.get('full') || u.searchParams.get('expanded') || '').toLowerCase();
...
function setUrlExpanded(full: boolean, push = false) {
    try {
        const u = new URL(location.href);
        if (full) u.searchParams.set('full', '1'); else u.searchParams.delete('full');
        if (push) history.pushState({ full }, "", u.toString()); else history.replaceState({ full }, "", u.toString());
    } catch { /* noop */ }
}
function installAuxStyles() {
    cy.style()
        .selector('.hidden-by-filter').style({ display: 'none' })
        .selector('.dim').style({ opacity: 0.12, 'z-index': 1 }) // dimmed nodes below active nodes
        .selector('edge.dim').style({ 'z-index': 0 }) // dimmed edges below dimmed nodes for easier node selection
        // All in-focus neighborhood (focus + ring) should stack above dimmed
        .selector('.focus-ring').style({ 'z-index': 600 })
        .selector('.focus-node').style({ 'border-width': 3, 'border-color': '#111827', 'z-index': 999 })
        .update();
}
installAuxStyles();
...
function focusSearchAfterExpand() {
    const attempt = (n: number) => {
        try {
            ...
            q.select?.();
            if (document.activeElement !== q && n < 3) setTimeout(() => attempt(n + 1), 60);
        } catch { }
    };
...
if (!sidepanel.classList.contains('collapsed')) {
    setTimeout(() => { try { q?.focus({ preventScroll: true } as any); q?.select?.(); } catch { } }, 0);
}
spCollapse?.addEventListener('click', () => {
    ...
    if (zoomBar) zoomBar.classList.toggle('horizontal', expanded);
    try { localStorage.setItem('qm.sidebar', expanded ? 'open' : 'collapsed'); } catch { }
...
const showOpposite = document.getElementById("show-opposite") as HTMLInputElement;
const showSupported = document.getElementById("show-supported") as HTMLInputElement;
...
function applyEdgeToggles() {
    ...
    cy.edges("[type='supportedBy']").toggleClass("hidden", !showSupported.checked);
    cy.edges("[type='leadsTo']").toggleClass("hidden", !showLeads.checked);
}
[showRelated, showOpposite, showSupported, showLeads].forEach(el => el?.addEventListener("change", applyEdgeToggles));
// Initialize toggles using server-provided defaults
try {
    const opts = JSON.parse(document.getElementById("qualities-map-options")?.textContent || "{}");
    if (opts?.flags) {
        if (typeof opts.flags.related === "boolean") showRelated.checked = !!opts.flags.related;
        if (typeof opts.flags.opposite === "boolean") showOpposite.checked = !!opts.flags.opposite;
    }
} catch { }
applyEdgeToggles();

// Node filters (checkboxes) for Wholesome/Unwholesome/Neutral with live counts
const nodeShowBright = document.getElementById("node-show-bright") as HTMLInputElement;
const nodeShowDark = document.getElementById("node-show-dark") as HTMLInputElement;
const nodeShowNeutral = document.getElementById("node-show-neutral") as HTMLInputElement;
...
function applyNodeFilters() {
    ...
    cy.nodes().forEach(n => {
        ...
        const hide = (pol === "positive" && !showPos) || (pol === "negative" && !showNeg) || (pol === "neutral" && !showNeu);
    ...
    if (countBright) countBright.textContent = String(cy.nodes("[polarity = 'positive']").not('.hidden').length);
    if (countDark) countDark.textContent = String(cy.nodes("[polarity = 'negative']").not('.hidden').length);
    if (countNeutral) countNeutral.textContent = String(cy.nodes("[polarity = 'neutral']").not('.hidden').length);
}
nodeShowBright?.addEventListener('change', () => { applyNodeFilters(); const vis = cy.elements().not('.hidden'); if (vis.nonempty()) cy.fit(vis, 80); });
nodeShowDark?.addEventListener('change', () => { applyNodeFilters(); const vis = cy.elements().not('.hidden'); if (vis.nonempty()) cy.fit(vis, 80); });
nodeShowNeutral?.addEventListener('change', () => { applyNodeFilters(); const vis = cy.elements().not('.hidden'); if (vis.nonempty()) cy.fit(vis, 80); });
applyNodeFilters();
...
function highlightNeighborhood(node: cytoscape.NodeSingular, depth: number) {
    cy.batch(() => {
        cy.elements().removeClass('dim focus-node from-focus to-focus focus-ring');
        ...
        cy.elements().forEach(e => { if (!keep.has(e.id())) e.addClass('dim'); });
...
function layoutFocusArea(node: cytoscape.NodeSingular, depth: number) {
    try {
        ...
        for (let d = 1; d <= depth; d++) {
            ...
            frontier.forEach(n => {
                n.neighborhood('node').forEach(m => {
                    if (!dist.has(m.id())) { dist.set(m.id(), d); next.merge(m); }
                });
            });
            frontier = next;
        }
        const nodes = cy.collection(Array.from(dist.keys()).map(id => cy.getElementById(id))).nodes();
        ...
        return new Promise<void>((resolve) => {
            ...
            const bbox = { x1: pad, y1: pad, w: cy.width() - pad * 2, h: cy.height() - pad * 2 } as any;
...
function collateAscii(a: string, b: string) {
    const aa = toAscii(a || ''), bb = toAscii(b || '');
    return aa < bb ? -1 : aa > bb ? 1 : 0;
}
function relFromFocus(focus: cytoscape.NodeSingular, n: cytoscape.NodeSingular): 'leadsTo' | 'supportedBy' | 'related' | 'opposite' | 'other' {
    const between = focus.edgesWith(n);
    // Directional precedence
    if (between.some(e => e.data('type') === 'leadsTo' && e.source().id() === focus.id())) return 'leadsTo';
    if (between.some(e => e.data('type') === 'leadsTo' && e.target().id() === focus.id())) return 'supportedBy';
    if (between.some(e => e.data('type') === 'supportedBy' && e.target().id() === focus.id())) return 'supportedBy';
    if (between.some(e => e.data('type') === 'supportedBy' && e.source().id() === focus.id())) return 'leadsTo';
...
function layoutFocusAreaBlock360(node: cytoscape.NodeSingular, depth: number) {
    try {
        ...
        dist.forEach((d, id) => { if (d > 0 && d <= maxDepth) { const arr = rings.get(d) || []; arr.push(id); rings.set(d, arr); } });
        ...
        rings.forEach((ids, d) => {
            ...
            (Object.keys(groups) as (keyof typeof groups)[]).forEach(k => {
                groups[k].sort((a, b) => collateAscii(String(a.data('label') || a.id()), String(b.data('label') || b.id())));
            });
        ...
        try {
            ...
            Object.keys(positions).forEach(id => {
                ...
                const dx = p.x - cx; const dy = p.y - cyy;
                positions[id] = { x: cx + dx * cos - dy * sin, y: cyy + dx * sin + dy * cos };
            });
        } catch { /* noop */ }

        // Run preset layout
        return new Promise<void>((resolve) => {
            const nodes = cy.collection(Object.keys(positions).map(id => cy.getElementById(id)));
            const layout = nodes.layout({ name: 'preset', positions: (ele: any) => positions[ele.id()], fit: false, animate: true, animationDuration: FOCUS_LAYOUT.animMs, animationEasing: 'ease-in-out-cubic', boundingBox: bbox } as any);
            layout.on('layoutstop', () => resolve());
            layout.run();
        });
    } catch {
...
function fitAroundNode(node: cytoscape.NodeSingular, depth: number) {
    ...
    cy.animate({ fit: { eles, padding: FOCUS_LAYOUT.padding } }, { duration: 280, easing: 'ease-in-out' });
}
function defaultFocusedView() {
    // pick highest-degree node
    let pick: any = null, best = -1;
    cy.nodes().forEach(n => { const d = n.degree(false); if (d > best) { best = d; pick = n; } });
...
function applyInitialFocus() {
    try {
        ...
        if (key) {
            ...
            if (n) { setFocus(n, { openDrawer: false, updateUrl: true, pushUrl: false }); return; }
            // Fallback: treat unknown focus key as a search term
            try { runSearchForTerm(key, { fromUrl: false, allowUrlRewrite: true }); return; } catch { /* noop */ }
        }
    } catch { }
    defaultFocusedView();
}
function clearSearchFilters() {
    searchActive = false;
    cy.batch(() => { cy.elements().removeClass('hidden-by-filter'); });
}
function effectiveDepth(input?: number): number {
    const fromOpts = (window as any).__QUAL_MAP_OPTS__?.depth;
    const d = typeof input === 'number' ? input : (typeof fromOpts === 'number' ? fromOpts : undefined);
    return d && d > 0 ? d : DEFAULT_FOCUS_DEPTH;
}

function setFocus(node: cytoscape.NodeSingular, opts?: { openDrawer?: boolean; updateUrl?: boolean; pushUrl?: boolean; depth?: number }) {
...
function installPopstateFocus() {
    window.addEventListener('popstate', () => {
        ...
        try {
            ...
            const nowFull = document.documentElement.classList.contains('qm-full');
            if (shouldFull !== nowFull) applyVpToggleState(shouldFull);
        } catch { }
        const key = getUrlFocus();
        if (!key) { currentFocusId = null; cy.elements().removeClass('dim focus-node'); clearSearchFilters(); defaultFocusedView(); return; }
        const n = findNodeByFocusKey(key);
        if (n) { setFocus(n, { openDrawer: false, updateUrl: false }); return; }
        // Fallback to treating the key as a search term, but do not rewrite URL during popstate
        try { runSearchForTerm(key, { fromUrl: false, allowUrlRewrite: false }); } catch { /* noop */ }
    });
}
// Initialize fullscreen class from URL before focusing/layout
try { if (getUrlExpanded()) document.documentElement.classList.add('qm-full'); } catch { }
applyInitialFocus();
...
(() => {
    if (!q) return;
    const MAP: Record<string, string> = { 'a': 'ā', 'i': 'ī', 'u': 'ū', 'n': 'ñ', 't': 'ṭ', 'd': 'ḍ', 'm': 'ṃ', 'h': 'ḥ', 'N': 'Ñ', 'A': 'Ā', 'I': 'Ī', 'U': 'Ū', 'T': 'Ṭ', 'D': 'Ḍ', 'M': 'Ṃ', 'H': 'Ḥ' };
    ...
    q.addEventListener('keydown', (ev: KeyboardEvent) => {
        ...
        const k = ev.key;
        if (!MAP[k]) { lastKey = null; if (pressTimer) { clearTimeout(pressTimer); pressTimer = null; } return; }
        // Stop OS/browser key repeat from inserting aaaaa...
        if ((ev as any).repeat) { ev.preventDefault(); return; }
        lastKey = k;
        ...
        pressTimer = window.setTimeout(() => {
            ...
            try {
                ...
                try { q.setSelectionRange(newPos, newPos); } catch { }
...
function maybeCollapsePanelOnSmallViewport(singleExact: boolean) {
    try {
        ...
        const btn = document.getElementById('sp-collapse') as HTMLButtonElement | null;
        if (panel && !panel.classList.contains('collapsed')) {
            panel.classList.add('collapsed');
            btn?.setAttribute('aria-expanded', 'false');
            if (btn) btn.textContent = '⟩';
            try { localStorage.setItem('qm.sidebar', 'collapsed'); } catch { }
            console.debug('[QM][panel] auto-collapsed on small viewport');
        }
    } catch { }
}
function clearSearchVisual() { cy.elements().removeClass("dimmed highlight dim focus-node"); clearSearchFilters(); setUrlFocus(null, false); currentFocusId = null; }
    ...
    for (let d = 0; d < depth; d++) {
        ...
        frontier.forEach(id => {
            ...
            n.connectedEdges().forEach(e => {
                ...
                if (!seen.has(v)) { seen.add(v); next.add(v); }
            });
        });
        frontier = next;
    }
...
q.addEventListener("keydown", (ev: KeyboardEvent) => {
    ...
    if (uniqIds.length > 1) {
        ...
        if (preferred) {
            ...
            if (node && node.nonempty && node.nonempty()) {
                ...
                clearSearchFilters();
                setFocus(node as any, { openDrawer: wasOpen, updateUrl: true, pushUrl: true });
    ...
    if (uniqIds.length === 1) {
        ...
        if (node && node.nonempty && node.nonempty()) {
            console.debug('[QM][enter] single-result focus=', uniqIds[0], 'preserveDrawerOpen=', wasOpen);
            clearSearchFilters();
            setFocus(node as any, { openDrawer: wasOpen, updateUrl: true, pushUrl: true });
            maybeCollapsePanelOnSmallViewport(true);
            return;
        ...
        if (fallback) { console.debug('[QM][enter] fallback direct focus preserveDrawerOpen=', wasOpen); clearSearchFilters(); setFocus(fallback, { openDrawer: wasOpen, updateUrl: true, pushUrl: true }); maybeCollapsePanelOnSmallViewport(true); return; }
    }
    ...
    const expanded = !sidepanel.classList.contains('collapsed');
    if (expanded) {
        setTimeout(() => { try { q?.focus(); q?.select?.(); } catch { } }, 0);
    }
    matchNodes.forEach(m => { pool = pool.union(m.neighborhood('node')); });
    const visibleNodeIds = new Set<string>();
    matchNodes.forEach(m => visibleNodeIds.add(m.id()));
    const poolArr = pool.nodes().toArray().sort((a, b) => b.degree(false) - a.degree(false));
    for (const n of poolArr) { if (visibleNodeIds.size >= 20) break; visibleNodeIds.add(n.id()); }
    searchActive = true; currentFocusId = null; setUrlFocus(null, false);
    cy.batch(() => {
        // Auto-focus when expanded via toggle
        if (expanded) {
            setTimeout(() => { try { q?.focus(); q?.select?.(); } catch { } }, 0);
        }
        cy.nodes().forEach(n => { if (visibleNodeIds.has(n.id())) n.removeClass('hidden-by-filter'); else n.addClass('hidden-by-filter'); });
        cy.edges().forEach(e => {
            const sVis = !e.source().hasClass('hidden-by-filter');
            const tVis = !e.target().hasClass('hidden-by-filter');
            if (sVis && tVis) e.removeClass('hidden-by-filter'); else e.addClass('hidden-by-filter');
        });
    ...
    cy.animate({ fit: { eles: vis, padding: FOCUS_LAYOUT.padding } }, { duration: 260 });
});
...
function runSearchForTerm(term: string, opts?: { fromUrl?: boolean; allowUrlRewrite?: boolean }) {
    try {
        if (q) {
            q.value = term;
    ...
    try {
        ...
        if (pref) {
            ...
            if (!node || !(node as any).nonempty || !(node as any).nonempty()) {
                const bySlug = cy.nodes().filter(n => toAscii(slugify(n.id())) === pref || toAscii(n.data('label')) === pref || toAscii(n.id()) === pref);
                if (bySlug && (bySlug as any).nonempty && (bySlug as any).nonempty()) node = (bySlug as any)[0];
            }
            if (node && (node as any).nonempty && (node as any).nonempty()) {
                clearSearchFilters();
                setFocus(node as any, { openDrawer: false, updateUrl: true, pushUrl: !!opts?.fromUrl });
                if (opts?.allowUrlRewrite) setUrlFocus(slugify((node as any).id()), false);
                maybeCollapsePanelOnSmallViewport(true);
                return;
            }
        }
    ...
    if (uniqIds.length > 1) {
        ...
        if (preferred) {
            ...
            if (node && (node as any).nonempty && (node as any).nonempty()) {
                clearSearchFilters();
                setFocus(node as any, { openDrawer: wasOpen, updateUrl: true, pushUrl: !!opts?.fromUrl });
                if (opts?.allowUrlRewrite) setUrlFocus(slugify(preferred.id), false);
                maybeCollapsePanelOnSmallViewport(true);
                return;
            }
        }
    ...
    if (uniqIds.length === 1) {
        ...
        if (fb) { clearSearchFilters(); setFocus(fb as any, { openDrawer: wasOpen, updateUrl: true, pushUrl: !!opts?.fromUrl }); if (opts?.allowUrlRewrite) setUrlFocus(slugify(fb.id()), false); maybeCollapsePanelOnSmallViewport(true); return; }
    }
    if (!results.length) return;
...
qGo?.addEventListener('click', () => {
    ...
    if (uniqIds.length > 1) {
        ...
        if (preferred) {
            const node = cy.getElementById(preferred.id);
            if (node && node.nonempty && node.nonempty()) {
                console.debug('[QM][enter-icon] pref-target focus=', preferred.id);
                clearSearchFilters();
                setFocus(node as any, { openDrawer: wasOpen, updateUrl: true, pushUrl: true });
    ...
    if (uniqIds.length === 1) {
        const node = cy.getElementById(uniqIds[0]);
        if (node && node.nonempty && node.nonempty()) {
            console.debug('[QM][enter-icon] single-result focus=', uniqIds[0], 'preserveDrawerOpen=', wasOpen);
            clearSearchFilters();
            setFocus(node as any, { openDrawer: wasOpen, updateUrl: true, pushUrl: true });
            maybeCollapsePanelOnSmallViewport(true);
            return;
        ...
        const fallback = findNodeByFocusKey(term);
        if (fallback) {
            console.debug('[QM][enter-icon] fallback direct focus preserveDrawerOpen=', wasOpen);
            clearSearchFilters();
            setFocus(fallback, { openDrawer: wasOpen, updateUrl: true, pushUrl: true });
            maybeCollapsePanelOnSmallViewport(true);
            return;
        }
    }
    ...
    const vis = cy.elements().filter(e => !e.hasClass('hidden-by-filter'));
    console.debug('[QM][enter-icon] multi-match ids=', Array.from(visibleNodeIds));
    cy.animate({ fit: { eles: vis, padding: FOCUS_LAYOUT.padding } }, { duration: 260 });
});
...
(() => { try { const has = !!(q?.value || '').trim(); if (qGo) qGo.classList.toggle('hidden', !has); } catch { } })();

// Accessibility: collapse the panel when Esc is pressed while focus is within sidepanel
document.getElementById('sidepanel')?.addEventListener('keydown', (ev: KeyboardEvent) => {
...
const vpToggle = document.getElementById('vp-toggle') as HTMLButtonElement | null;
...
zoomReset?.addEventListener("click", () => {
    ...
    let pick: any = null, best = -1;
    nodes.forEach(n => { const d = n.degree(false); if (d > best) { best = d; pick = n; } });
    if (pick) setFocus(pick, { openDrawer: false, updateUrl: false }); else cy.fit(cy.elements(), 80);
});
...
function applyVpToggleState(full: boolean) {
    ...
    if (full) {
        try { prevView = { zoom: cy.zoom(), pan: cy.pan() }; } catch { prevView = null; }
        if (currentFocusId) {
            ...
            const vis = cy.elements().filter(e => !e.hasClass('hidden') && !e.hasClass('hidden-by-filter'));
            if (vis.nonempty && vis.nonempty()) cy.fit(vis, 40); else cy.fit(cy.elements(), 40);
        }
    } else {
        ...
        if (currentFocusId) {
            ...
            const vis = cy.elements().filter(e => !e.hasClass('hidden') && !e.hasClass('hidden-by-filter'));
            if (vis.nonempty && vis.nonempty()) cy.fit(vis, 80); else cy.fit(cy.elements(), 80);
        }
        prevView = null;
    }
}
...
if (vpToggle) {
    const full = document.documentElement.classList.contains('qm-full');
    vpToggle.textContent = full ? '⤡' : '⤢';
    vpToggle.setAttribute('aria-label', full ? 'Collapse to normal view' : 'Expand to fullscreen');
...
function transformId(id: string): string {
    ...
    const keyMap: Record<string, string> = { dhp: "The Path of Dhamma", mn: "Middle Length Discourses", ud: "Inspired Utterances", sn: "Linked Discourses", snp: "The Buddha's Ancient Discourses", an: "Numerical Discourses", iti: "As It Was Said", kp: "Minor Passages", anthologies: "Anthologies", "noble-truths-noble-path": "Noble Truths, Noble Path", "in-the-buddhas-words": "In the Buddha's Words" };
    id = keyMap[id] || id;
    const transformed = id.replace(/([a-zA-Z]+)(\d+)/, (_: any, chars: string, digits: string) => `${chars.toUpperCase()} ${digits}`);
    if (transformed === id) { const decoded = decodeURIComponent(id); return decoded.charAt(0).toUpperCase() + decoded.slice(1); }
    return transformed;
}
function updatePostLinks(container: HTMLElement) {
    ...
    links.forEach((link) => {
        const baseHref = (link as HTMLAnchorElement).getAttribute("data-base-href") || (link as HTMLAnchorElement).href;
        const params = new URLSearchParams();
        if (paliMode) params.set("pli", "true");
        if (layout === "split") params.set("layout", layout);
        (link as HTMLAnchorElement).href = params.toString() ? `${baseHref}?${params.toString()}` : (baseHref || '');
    });
}
                                ...
                                <div class="flex items-start flex-grow">
                                    <div class="min-w-0 pr-4">
                                        <h2 class="text-base sm:text-lg font-semibold text-text mt-2 mb-2">
                                            <a href="/${d.id}" class="post-link text-gray-500 hover:text-link-color id mr-2 font-normal" data-base-href="/${d.id}">
                                                ${transformId(d.id)}&nbsp;<span style="color:var(--text-color)">${d.title}</span>
                                            </a>
                                            ${note ? `<span class="px-2 py-1 text-xs rounded-full font-normal bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 whitespace-nowrap inline-block align-bottom -translate-y-0.5 mt-1">${note}</span>` : ''}
                                        </h2>
                                    </div>
                                </div>
                            </div>
                            <p class="mt-2 text-text text-sm sm:text-base">${(d.description || '').replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-blue-600 hover:underline">$1</a>')}</p>
                        </div>`;
    }).join('');
...
function initInfiniteScroll() {
    ...
    }, { root: document.querySelector('#drawer .body'), rootMargin: '200px', threshold: 0 });
...
function tagHtmlForQuality(slug: string): string {
    ...
    const cls = isPos ? 'topic-tag positive' : isNeg ? 'topic-tag negative' : 'topic-tag neutral';
...
function renderDrawer(nodeId: string) {
    ...
    const id = n.id(); const label = n.data("label"); const pol = n.data("polarity");
    const meta = built.meta[id] || { lines: [] };
    const slug = slugify(id);
    titleEl.textContent = label;
    titleEl.href = `/on/${slug}`;
    const polType = pol === 'positive' ? 'bright-quality' : pol === 'negative' ? 'negative-quality' : 'neutral-quality';
    ...
    const getList = (label: RegExp) => {
        return lines
            ...
            .filter(Boolean)
            .map((m: any) => m[1])
            .flatMap((body: string) => body.split(',').map(s => s.trim()).filter(Boolean));
    };
    const stripBrackets = (s: string) => s.replace(/[{}\[\]]/g, '').trim();
    const alsoKnown = lines
        .filter(l => !/^(Supported by|Leads to|Related|Related to|Opposite|Context)\s*:/i.test(l) && !/^\[.*\]$/.test(l))
        .map(stripBrackets)
        .filter(Boolean);
    const pali = lines
        .map(l => l.match(/^\s*\[([^\]]+)\]\s*$/))
    ...
    const setRow = (wrap: HTMLElement, row: HTMLElement, arr: string[]) => {
        if (!wrap || !row) return;
        if (arr.length) { wrap.style.display = ""; row.innerHTML = arr.map(tagHtmlForQuality).join(''); }
        else { wrap.style.display = "none"; row.innerHTML = ""; }
    };
    ...
    fetch(apiUrl.toString())
        ...
        .then(result => {
            const item = (result?.data || []).find((x: any) => {
                ...
                return x.slug.toLocaleLowerCase() === slug.toLocaleLowerCase() || x.slug.toLocaleLowerCase() === slug.replaceAll('-', ' ').toLocaleLowerCase()
            });
            const disc = item?.discourses || [];
            currentDiscourses = disc;
            discCards.innerHTML = disc.length ? '' : `<div class="soft">No discourses found.</div>`;
            if (disc.length) { renderCardsNextPage(); initInfiniteScroll(); }
        })
        .catch(() => { discCards.innerHTML = `<div class="soft">Could not load discourses.</div>`; });
}
...
document.getElementById('drawer')?.addEventListener('click', (ev) => {
    ...
    const slug = a.getAttribute('data-focus') || (a.search && new URLSearchParams(a.search).get('focus')) || undefined;
    const mouse = ev as MouseEvent;
    const altOpen = mouse?.metaKey || mouse?.ctrlKey || mouse?.button === 1;
    console.debug('[QM][drawer-click]', { slug, altOpen, href: a.getAttribute('href') });
    if (slug && !altOpen) {
        ev.preventDefault();
        ev.stopPropagation();
        ...
        if (n) {
            ...
            setFocus(n, { openDrawer: true, updateUrl: true, pushUrl: true });
        } else {
...
function openExplorerPanel() {
    try {
        ...
        if (!sidepanel.classList.contains('collapsed')) {
            ...
            try { q?.focus({ preventScroll: true } as any); q?.select?.(); } catch { }
            return;
        }
        ...
        try { localStorage.setItem('qm.sidebar', 'open'); } catch { }
...
cy.on("tap", (e) => {
    ...
    const pos = e.position || (e.renderedPosition ? cy.renderer().projectIntoViewport(e.renderedPosition.x, e.renderedPosition.y) : null);
    // Fallback: if cytoscape already targeted a node, use that
    let target = (e.target && (e.target as any).isNode && (e.target as any).isNode()) ? (e.target as any) : null;
    if (!target && pos) {
        const candidates = cy.elementsAtPoint(pos.x, pos.y).filter('node');
        if (candidates.nonempty()) {
            const visibleFirst = candidates.filter(n => !n.hasClass('hidden-by-filter') && !n.hasClass('hidden'));
            const nonDim = visibleFirst.filter(n => !n.hasClass('dim'));
            target = (nonDim.nonempty() ? nonDim[0] : visibleFirst.nonempty() ? visibleFirst[0] : candidates[0]) as any;
        }
    }
    if (target && target.isNode && target.isNode()) {
        ...
        if (lastTap.id === id && now - lastTap.t < 350) {
            setFocus(target as any, { openDrawer: false, updateUrl: true, pushUrl: true });
