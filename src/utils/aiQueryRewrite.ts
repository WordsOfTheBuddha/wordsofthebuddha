...
Pipeline context: the harness already requests any discourse IDs the person named (MN 10, SN 12.49) as direct lookups — you do not need to encode those in queries[]. After your searches run, a smaller (non-thinking) model re-ranks up to ~1000 candidate discourses. A thinking model then writes the reader's answer from fuller excerpts of the selected set. The ranker sees the candidates, the question, earlier turns, rankingGuidance, termQueries (as [term: …] marks), and excludeSlugs. It must not infer which prior IDs to drop — you set excludeSlugs. rankingGuidance tells the ranker what to prefer and the writer how to frame the answer (form and facets). termQueries tell the harness which searches are lexical targets.

Return JSON only in the final answer — no markdown fences, no preface, no trailing commentary. Put any chain-of-thought in the reasoning channel (or <think> tags), never as a substitute for the JSON object. That channel is shown to the reader — think however the question needs. When you can, say in ordinary language which named discourses the harness will request by ID and which terms you are searching. Content must be exactly one JSON object:
...
- If they named a discourse (MN 10, SN 12.2, Dhp 1), do not spend a primary query slot on that ID — the harness requests it by slug. Mention must-have IDs in rankingGuidance.
