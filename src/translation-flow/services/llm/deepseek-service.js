...
export class DeepseekService {
  constructor() {
    ...
    if (!this.apiKey) {
      console.warn(chalk.yellow('DEEPSEEK_API_KEY environment variable not set. DeepSeek service may not work.'));
    }
  }
  ...
  safeJsonParse(text) {
    try {
      ...
      if (text.startsWith("'") && text.endsWith("'")) {
        ...
        console.log("Attempting to fix JSON: Removing surrounding single quotes");
      }
      
      // Check for unescaped newlines in strings
      if (fixedText.includes('"\n"')) {
        fixedText = fixedText.replace(/"\n"/g, '\\n');
        console.log("Attempting to fix JSON: Escaping newlines in strings");
      }
  ...
  async translate(prompt, options = {}) {
    ...
    while (retries < MAX_RETRIES) {
      try {
        ...
        if (retries > 0) {
          console.log(chalk.yellow(`Retry attempt ${retries}/${MAX_RETRIES}...`));
          await this.sleep(RETRY_DELAY * retries); // Exponential backoff
        }
        
        console.log(chalk.dim("Making API request to DeepSeek..."));
        const response = await fetch(`${API_BASE_URL}/${API_VERSION}/chat/completions`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          ...
          console.error(chalk.red(`API error response: ${errorText}`));
          throw new Error(`DeepSeek API error: ${response.status} - ${errorText}`);
        }
        
        // Read text from response
        const text = await response.text();
        console.log(chalk.dim(`Received ${text.length} bytes from API`));
        ...
        const debugFile = path.join(debugDir, `deepseek-response-${Date.now()}.json`);
        ...
        if (!firstChoice || !firstChoice.message || !firstChoice.message.content) {
          console.error("Unexpected choice structure:", JSON.stringify(firstChoice, null, 2));
        ...
        lastError = error;
        console.error(chalk.red(`DeepSeek API error (attempt ${retries + 1}/${MAX_RETRIES}): ${error.message}`));
        retries++;
    ...
    throw new Error(`DeepSeek translation failed after ${MAX_RETRIES} attempts: ${lastError.message}`);
  }
}